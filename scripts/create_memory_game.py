...
// Culturally relevant items with emoji and name
const ITEMS = [
  { emoji: '🍎', name: 'Apple' },
  { emoji: '🏠', name: 'House' },
  { emoji: '🌸', name: 'Flower' },
...
function MemoryGame({ onExit }) {
  ...
  const [phase, setPhase] = useState('setup'); // setup, show, select, finished
  ...
  const handleSubmit = () => {
    ...
    selected.forEach((selectedName) => {
      ...
      answers.push({
        ...
        reaction_time_ms: 0, // we don't track per item here
      });
    });

    // Record each answer in session
    answers.forEach((a) => {
      ...
      <div className="w-full max-w-3xl">
        ...
        {phase === 'setup' && (
          <div className="bg-white rounded-2xl shadow-xl p-8 text-center">
            <h1 className="text-4xl font-bold text-blue-800 mb-6">Memory Game</h1>
            <p className="text-xl text-gray-600 mb-8">Choose difficulty to begin</p>
        ...
        {phase === 'show' && (
          <div className="bg-white rounded-2xl shadow-xl p-8 text-center">
            <h2 className="text-3xl font-bold text-blue-800 mb-6">Remember these objects!</h2>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {displayItems.map((item) => (
                <div
                  key={item.name}
                  className="bg-blue-50 rounded-xl p-6 flex flex-col items-center justify-center animate-fade-in"
                >
        ...
        {phase === 'select' && (
          <div className="bg-white rounded-2xl shadow-xl p-8 text-center">
            <h2 className="text-3xl font-bold text-blue-800 mb-4">Which objects did you see?</h2>
            <p className="text-lg text-gray-600 mb-6">Select all that you remember</p>
        ...
        {phase === 'finished' && (
          <div className="bg-white rounded-2xl shadow-xl p-8 text-center">
            <h2 className="text-4xl font-bold text-blue-800 mb-6">Game Over!</h2>
...
print("MemoryGame.jsx created successfully!")
