...
const DIFFICULTY_SETTINGS = {
  easy: { patternLength: 4, optionsCount: 3, timeLimit: 15 },
  medium: { patternLength: 6, optionsCount: 4, timeLimit: 12 },
  hard: { patternLength: 8, optionsCount: 5, timeLimit: 10 },
};
...
function generatePattern(length) {
  ...
  for (let i = 0; i < length; i++) {
    pattern.push(availableColors[i % 2]);
  }
  return pattern;
}
...
function PatternGame({ onExit }) {
  ...
  // Countdown effect: runs only when phase is 'question'
  useEffect(() => {
...
print("PatternGame.jsx fixed with useEffect timer!")
